...
my $BuildPath = "\\\\10.4.13.86\\jenkins\\jobs\\BluRing_1.0_Python\\builds";
my $TestDataPath = "\\\\10.4.16.130\\BluRing";
...
my @currentdaybuilds;
my $Destinationpath;
my $Imgdrvpath = "D:\\imgdrv";
my $smtphost="10.5.16.2";
my $currentdate = strftime "%Y-%m-%d", localtime;
#my $currentdate = "2017-03-18";
my @TestResultsfile;
my $Attachements;
my $SenderList = "lakshman.shiva\@aspiresys.com,ptate\@ca.ibm.com,rajesh.kalaivanan\@Aspiresys.com,guptapu\@us.ibm.com,Jack.Pei\@ca.ibm.com,Stephen.Zielinski\@ca.ibm.com,Rob.Mitchell\@ca.ibm.com,Anubha.Tiwari\@ca.ibm.com,karls\@ca.ibm.com,velu.nallasamy\@Aspiresys.com";
...
{
  print Log GetLoggingTime()."Mapping Test Data Directory\n";	
  print GetLoggingTime()."Mapping Test Data Directory\n";
  
  my $command_mapdrive = "net USE Y: ".$TestDataPath." ".$Password_Data." "."\/USER\:$Domain_User_Data $PERSISTENT";
  my $command_removedriver = "net use /delete /y "."Y:";
  system($command_removedriver);
  system($command_mapdrive); 
...
if (-e $TestDataPath && -d $TestDataPath) 
  ...
  ###Create Hardlink in D drive for above mapped drive
  system("mkdir D:\\Data");
  system("mklink /d D:\\Data\\BluRing Y:\\");
  
  if (-e "D:\\Data\\BluRing\\Lung" && -d "D:\\Data\\BluRing\\Lung") 
  {
  	print Log GetLoggingTime()."Hardlink created\n";
  	print GetLoggingTime()."Hardlink created\n";
  }
  else
  {
  	print Log GetLoggingTime()."Hardlink created\n";
...
{   
 ...
 else
 {
 	...
 	#Setup SMTP Server
    system("$Imgdrvpath\\blat -install $smtphost admin\@merge.com 1 25");    	
    $commandsendemail = "$Imgdrvpath\\blat $emailbody -to $SenderList -subject BlueRing_NoBuildFound -log";
...
{  
  ...
  $Destinationpath = "D:\\BlueRingBuilds"."\\".$CurrBuildPath."\\*\.*";
